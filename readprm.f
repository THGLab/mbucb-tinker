...
c
      ...
      integer ft(6),pg(maxval),pg_ct(maxval)
...
c
     ...
     &              (use_anisopolz ) ) then
            ...
            string = record(next:180)
            read (string,*,err=451,end=451)  ia,anisopol(1),anisopol(2),
  ...
  451       continue
     ...
     &              (use_anisopolz ) ) then
c            print*,"readprm in correct branch"
            ia = 0
            pol=0.0d0
            do jj=1,9
               anisopol(jj) = 0.0d0
            ...
            string = record(next:120)
            read (string,*,err=452,end=452)  ia,pol,thl,
     &                                       (pg(i),i=1,maxval)
  452       continue
            if (ia .ne. 0) then
               if(ia.eq.247.or.ia.eq.248) then
c                 print*,"readprm HELLO ia=",ia,"polz=",pol
               end if
               do jj=1,9
                if((jj.eq.1).or.(jj.eq.5).or.(jj.eq.9)) then
                polr_aniso(jj,ia) = pol
                else
                polr_aniso(jj,ia) = 0.0d0
                end if
               end do
               athl(ia) = thl
               do i = 1, maxval
...
! qtw >> out
            ...
            pn =0.0d0
            string = record(next:120)
            read (string,*,err=459,end=459)  ia,pn
  459       continue
            if (ia .ne. 0) then
               penr(ia) = pn
            end if
            ...
            string = record(next:180)
!            read (string,*,err=457,end=457)  ia,cq_trans
c            read (string,*,err=457,end=457)  ia,cq_trans,thl_ct,beta_ct,
c     &                                       (pg_ct(i),i=1,maxval)
            read (string,*,err=457,end=457)  ia,cq_trans,thl_ct,beta_ct,
     &                                       (pg(i),i=1,maxval)
  457       continue
            if (ia .ne. 0) then
!               rpolarity_ct(ia) = pol_ct
               cqr_trans(ia) = cq_trans
               rthole_ct(ia) = thl_ct
               rbeta_charge(ia)= beta_ct
               do i = 1, maxval
c                  pgrp_ct(i,ia) = pg_ct(i)
                  pgrp(i,ia) = pg(i)
               end do
c                 print*,"readprm HELLO ia=",ia,"cq_trans=",cq_trans,
c     &                   thl_ct,beta_ct
            end if
