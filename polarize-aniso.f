...
c         write (iout,10)
c   10    format (/,' POLARIZE  --  Dipole Polarizability',
c     &              ' is Not in Use')
c         call fatal
c      end if
      addu = 0.0d0
...
c
      do i = 1, npole
            uind(1,i) = anisopolarity(1,i)*field(1,i) +
     &      anisopolarity(2,i)*field(2,i) 
     &      + anisopolarity(3,i)*field(3,i)

            uind(2,i) = anisopolarity(4,i)*field(1,i) + 
     &      anisopolarity(5,i)*field(2,i) 
     &      + anisopolarity(6,i)*field(3,i)

            uind(3,i) = anisopolarity(7,i)*field(1,i) +
     &      anisopolarity(8,i)*field(2,i)
     &      + anisopolarity(9,i)*field(3,i)

            do j=1,3
            uind(j,i) = uind(j,i)
            write(*,*)'bugg testt akd',uind(j,i)
            end do
      end do
c
c     perform dynamic allocation of some local arrays
