      subroutine induce0a_aniso_invert_test
      use sizes
      use atoms
      use inform
      use iounit
...
c
      print*,"In induce_aniso_invert_test"
      do i = 1, npole
         if(i.eq.3) then
           do j=1,9
             print*,"anisopol j=",j,ranisopolarity(j,i)
           end do
         end if 
         do j = 1, 3
            uind(j,i) = 0.0d0
...
c
c      call invert(3*npole,ranisopol_mat)
...
c      do i=1,3*npole
c         do j=1,3*npole
c            Cmat(j,i)=ranisopol_mat(j,i)+T_mut(j,i)
c         end do
c      end do
      ...
      do i=1,npole
         do j=1,3
           i1=3*(i-1)+j
           T_mut(i1,i1) = T_mut(i1,i1) + 1.0d0/ranisopolarity(1,i)
         end do
      end do
