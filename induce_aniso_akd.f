...
c
      if (debug) then
         header = .true.
         do i = 1, npole
            if (((anisopolarity(1,i)+anisopolarity(5,i)+
     &           anisopolarity(9,i))/3.0d0) .ne. 0.0d0) then
               if (header) then
                  header = .false.
...
c
      ...
      real*8, allocatable :: inv_ranisopolarity(:,:) 
      real*8, allocatable :: detinvpoli(:)
...
c
      if (use_ewald) then
         call dfield0c_aniso (field,fieldp)
      else if (use_mlist) then
         call dfield0b_aniso (field,fieldp)
      else
         call dfield0a_aniso (field,fieldp)
      end if
c
...
c     perform dynamic allocation of some local arrays
c
c      if(use_anisopolz) then
c       allocate (anisopolarity(9,n))
c       allocate (ranisopolarity(9,n))
c      else
c         allocate (poli(npole))
c      end if
         allocate (detinvpoli(npole))
         allocate (inv_ranisopolarity(9,npole))
...
c
         if (use_ewald) then
            call ufield0c_aniso (field,fieldp)
         else if (use_mlist) then
            call ufield0b_aniso (field,fieldp)
         else
            call ufield0a_aniso (field,fieldp)
         end if


c get inverse of the anisotropic polarizability 
         do i = 1,npole
           detinvpoli(i)=1.0d0/(
     &       ranisopolarity(1,i)*ranisopolarity(5,i)*ranisopolarity(9,i)
     &     - ranisopolarity(1,i)*ranisopolarity(6,i)*ranisopolarity(8,i)
     &     - ranisopolarity(2,i)*ranisopolarity(4,i)*ranisopolarity(9,i)
     &     + ranisopolarity(2,i)*ranisopolarity(6,i)*ranisopolarity(7,i)
     &     + ranisopolarity(3,i)*ranisopolarity(4,i)*ranisopolarity(8,i)
     &     - ranisopolarity(3,i)*ranisopolarity(5,i)*ranisopolarity(7,i)
     &                         )
c          write(*,*)ranisopolarity(1,i),ranisopolarity(2,i),
c     &              ranisopolarity(3,i)
c          write(*,*)ranisopolarity(4,i),ranisopolarity(5,i),
c     &              ranisopolarity(6,i)
c          write(*,*)ranisopolarity(7,i),ranisopolarity(8,i),
c     &              ranisopolarity(9,i)
c
c          
c          write(*,*)'DETINV',detinvpoli(i)
c
          inv_ranisopolarity(1,i) = detinvpoli(i)*(
     &          (ranisopolarity(5,i)*ranisopolarity(9,i) - 
     &          ranisopolarity(6,i)*ranisopolarity(8,i)))
          inv_ranisopolarity(2,i) = -detinvpoli(i)*(
     &          (ranisopolarity(2,i)*ranisopolarity(9,i) - 
     &          ranisopolarity(3,i)*ranisopolarity(8,i)))
          inv_ranisopolarity(3,i) = detinvpoli(i)*(
     &          (ranisopolarity(2,i)*ranisopolarity(6,i) - 
     &          ranisopolarity(3,i)*ranisopolarity(5,i)))
          inv_ranisopolarity(4,i) = -detinvpoli(i)*(
     &          (ranisopolarity(4,i)*ranisopolarity(9,i) - 
     &          ranisopolarity(6,i)*ranisopolarity(7,i)))
          inv_ranisopolarity(5,i) = detinvpoli(i)*(
     &          (ranisopolarity(1,i)*ranisopolarity(9,i) - 
     &          ranisopolarity(3,i)*ranisopolarity(7,i)))
          inv_ranisopolarity(6,i) = -detinvpoli(i)*(
     &          (ranisopolarity(1,i)*ranisopolarity(6,i) - 
     &          ranisopolarity(3,i)*ranisopolarity(4,i)))
          inv_ranisopolarity(7,i) = detinvpoli(i)*(
     &          (ranisopolarity(4,i)*ranisopolarity(8,i) - 
     &          ranisopolarity(5,i)*ranisopolarity(7,i)))
          inv_ranisopolarity(8,i) = -detinvpoli(i)*(
     &          (ranisopolarity(1,i)*ranisopolarity(8,i) - 
     &          ranisopolarity(2,i)*ranisopolarity(7,i)))
          inv_ranisopolarity(9,i) = detinvpoli(i)*(
     &          (ranisopolarity(1,i)*ranisopolarity(5,i) - 
     &          ranisopolarity(2,i)*ranisopolarity(4,i)))
c         write(*,*)inv_ranisopolarity(1,i),inv_ranisopolarity(2,i),
c     &             inv_ranisopolarity(3,i)
c         write(*,*)inv_ranisopolarity(4,i),inv_ranisopolarity(5,i),
c     &             inv_ranisopolarity(6,i)
c         write(*,*)inv_ranisopolarity(7,i),inv_ranisopolarity(8,i),
c     &             inv_ranisopolarity(9,i)
         end do
c
c     set initial conjugate gradient residual and conjugate vector
c
c         do i = 1, npole
c            poli(i) =max(polmin,((anisopolarity(1,i)+anisopolarity(5,i)+
c     &           anisopolarity(9,i))/3.0d0))
c            do j = 1, 3
...
c         end do
         do i = 1, npole
               rsd(1,i) = inv_ranisopolarity(1,i)*(udir(1,i)-uind(1,i))
     &                  + inv_ranisopolarity(2,i)*(udir(2,i)-uind(2,i))
     &                  + inv_ranisopolarity(3,i)*(udir(3,i)-uind(3,i))
     &                       + field(1,i)
               rsd(2,i) = inv_ranisopolarity(4,i)*(udir(1,i)-uind(1,i))
     &                  + inv_ranisopolarity(5,i)*(udir(2,i)-uind(2,i))
     &                  + inv_ranisopolarity(6,i)*(udir(3,i)-uind(3,i))
     &                       + field(2,i)
               rsd(3,i) = inv_ranisopolarity(7,i)*(udir(1,i)-uind(1,i))
     &                  + inv_ranisopolarity(8,i)*(udir(2,i)-uind(2,i))
     &                  + inv_ranisopolarity(9,i)*(udir(3,i)-uind(3,i))
     &                       + field(3,i)
             rsdp(1,i) =  inv_ranisopolarity(1,i)*(udirp(1,i)-uinp(1,i))
     &                  + inv_ranisopolarity(2,i)*(udirp(2,i)-uinp(2,i))
     &                  + inv_ranisopolarity(3,i)*(udirp(3,i)-uinp(3,i))
     &                       + fieldp(1,i)
             rsdp(2,i) =  inv_ranisopolarity(4,i)*(udirp(1,i)-uinp(1,i))
     &                  + inv_ranisopolarity(5,i)*(udirp(2,i)-uinp(2,i))
     &                  + inv_ranisopolarity(6,i)*(udirp(3,i)-uinp(3,i))
     &                       + fieldp(2,i)
             rsdp(3,i) =  inv_ranisopolarity(7,i)*(udirp(1,i)-uinp(1,i))
     &                  + inv_ranisopolarity(8,i)*(udirp(2,i)-uinp(2,i))
     &                  + inv_ranisopolarity(9,i)*(udirp(3,i)-uinp(3,i))
     &                       + fieldp(3,i)
         end do

         mode = 'BUILD'
         if (use_mlist) then
            call uscale0b_aniso (mode,rsd,rsdp,zrsd,zrsdp)
            mode = 'APPLY'
            call uscale0b_aniso (mode,rsd,rsdp,zrsd,zrsdp)
         else
            call uscale0a_aniso (mode,rsd,rsdp,zrsd,zrsdp)
            mode = 'APPLY'
            call uscale0a_aniso (mode,rsd,rsdp,zrsd,zrsdp)
         end if
         ...
         do while (.not. done)
            ...
            do i = 1, npole
               do j = 1, 3
                  ...
                  vec(1,i) = inv_ranisopolarity(1,i)*conj(1,i)
     &                     + inv_ranisopolarity(2,i)*conj(2,i) 
     &                     + inv_ranisopolarity(3,i)*conj(3,i) 
     &                     - field(1,i)
                  vec(2,i) = inv_ranisopolarity(4,i)*conj(1,i)
     &                     + inv_ranisopolarity(5,i)*conj(2,i) 
     &                     + inv_ranisopolarity(6,i)*conj(3,i) 
     &                     - field(2,i)
                  vec(3,i) = inv_ranisopolarity(7,i)*conj(1,i)
     &                     + inv_ranisopolarity(8,i)*conj(2,i) 
     &                     + inv_ranisopolarity(9,i)*conj(3,i) 
     &                     - field(3,i)
                 vecp(1,i) = inv_ranisopolarity(1,i)*conjp(1,i)
     &                     + inv_ranisopolarity(2,i)*conjp(2,i) 
     &                     + inv_ranisopolarity(3,i)*conjp(3,i) 
     &                     - fieldp(1,i)
                 vecp(2,i) = inv_ranisopolarity(4,i)*conjp(1,i)
     &                     + inv_ranisopolarity(5,i)*conjp(2,i) 
     &                     + inv_ranisopolarity(6,i)*conjp(3,i) 
     &                     - fieldp(2,i)
                 vecp(3,i) = inv_ranisopolarity(7,i)*conjp(1,i)
     &                     + inv_ranisopolarity(8,i)*conjp(2,i) 
     &                     + inv_ranisopolarity(9,i)*conjp(3,i) 
     &                     - fieldp(3,i)
...
c
c      if(use_anisopolz) then
c       deallocate (anisopolarity(9,n))
c       deallocate (ranisopolarity(9,n))
c      else
c         deallocate (poli(npole))
c      end if
         deallocate(detinvpoli)
         deallocate(inv_ranisopolarity)
         deallocate (rsd)
         deallocate (rsdp)
         deallocate (zrsd)
         deallocate (zrsdp)
...
c
c
      subroutine dfield0a_aniso (field,fieldp)
      use sizes
      use atoms
      use bound
      use cell
...
c
c
      subroutine ufield0a_aniso (field,fieldp)
      use sizes
      use atoms
      use bound
      use cell
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine dfield0b_aniso  --  direct induction via pair list  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "dfield0b_aniso" computes the mutual electrostatic field due to
c     permanent multipole moments via a pair list
c
c
      subroutine dfield0b_aniso (field,fieldp)
      use sizes
      use atoms
      use bound
      use couple
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine ufield0b_aniso  --  mutual induction via pair list  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "ufield0b_aniso" computes the mutual electrostatic field due to
c     induced dipole moments via a pair list
c
c
      subroutine ufield0b_aniso (field,fieldp)
      use sizes
      use atoms
      use bound
      use group
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine dfield0c_aniso  --  direct induction via Ewald sum  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "dfield0c_aniso" computes the mutual electrostatic field due to
c     permanent multipole moments via Ewald summation
c
c
      subroutine dfield0c_aniso (field,fieldp)
      use sizes
      use atoms
      use boxes
      use ewald
...
c
      if (use_mlist) then
         call udirect2b_aniso (field,fieldp)
      else
         call udirect2a_aniso (field,fieldp)
      end if
c
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine ufield0c_aniso  --  mutual induction via Ewald sum  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "ufield0c_aniso" computes the mutual electrostatic field due to
c     induced dipole moments via Ewald summation
c
c
      subroutine ufield0c_aniso (field,fieldp)
      use sizes
      use atoms
      use boxes
      use ewald
...
c     get the reciprocal space part of the electrostatic field
c
      call umutual1_aniso (field,fieldp)
c
c     get the real space portion of the electrostatic field
c
      if (use_mlist) then
         call umutual2b_aniso (field,fieldp)
      else
         call umutual2a_aniso (field,fieldp)
      end if
c
c     get the self-energy portion of the electrostatic field
c
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine udirect1_aniso  --  Ewald recip direct induced field  ##
c     ##                                                             ##
c     #################################################################
c
c
c     "udirect1_aniso" computes the reciprocal space contribution of the
c     permanent atomic multipole moments to the field
c
c
      subroutine udirect1_aniso (field)
      use sizes
      use bound
      use boxes
      use ewald
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine udirect2a_aniso  --  Ewald real direct field via loop  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "udirect2a_aniso" computes the real space contribution of the permanent
c     atomic multipole moments to the field via a double loop
c
c
      subroutine udirect2a_aniso (field,fieldp)
      use sizes
      use atoms
      use boxes
      use bound
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine udirect2b_aniso  --  Ewald real direct field via list  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "udirect2b_aniso" computes the real space contribution of the permanent
c     atomic multipole moments to the field via a neighbor list
c
c
      subroutine udirect2b_aniso (field,fieldp)
      use sizes
      use atoms
      use boxes
      use bound
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine umutual1_aniso  --  Ewald recip mutual induced field  ##
c     ##                                                             ##
c     #################################################################
c
c
c     "umutual1_aniso" computes the reciprocal space contribution of the
c     induced atomic dipole moments to the field
c
c
      subroutine umutual1_aniso (field,fieldp)
      use sizes
      use boxes
      use ewald
      use math
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine umutual2a_aniso  --  Ewald real mutual field via loop  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "umutual2a_aniso" computes the real space contribution of the induced
c     atomic dipole moments to the field via a double loop
c
c
      subroutine umutual2a_aniso (field,fieldp)
      use sizes
      use atoms
      use boxes
      use bound
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine umutual2b_aniso  --  Ewald real mutual field via list  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "umutual2b_aniso" computes the real space contribution of the induced
c     atomic dipole moments to the field via a neighbor list
c
c
      subroutine umutual2b_aniso (field,fieldp)
      use sizes
      use mpole
      use polar
      use tarray
...
c     ##############################################################
c     ##                                                          ##
c     ##  subroutine induce0d_aniso  --  Kirkwood SCRF induced dipoles  ##
c     ##                                                          ##
c     ##############################################################
c
c
c     "induce0d_aniso" computes the induced dipole moments at polarizable
c     sites for generalized Kirkwood SCRF and vacuum environments
c
c
      subroutine induce0d_aniso
      use sizes
      use atoms
      use inform
      use iounit
...
c
      call dfield0d_aniso (field,fieldp,fields,fieldps)
c
...
c     set initial conjugate gradient residual and conjugate vector
c
         call ufield0d_aniso (field,fieldp,fields,fieldps)
         do i = 1, npole
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine dfield0d_aniso  --  generalized Kirkwood direct field  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "dfield0d_aniso" computes the direct electrostatic field due to
c     permanent multipole moments for use with with generalized
c     Kirkwood implicit solvation
c
c
      subroutine dfield0d_aniso (field,fieldp,fields,fieldps)
      use sizes
      use atoms
      use couple
      use gkstuf
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine ufield0d_aniso  --  generalized Kirkwood mutual field  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "ufield0d_aniso" computes the mutual electrostatic field due to
c     induced dipole moments for use with with generalized Kirkwood
c     implicit solvation
c
c
      subroutine ufield0d_aniso (field,fieldp,fields,fieldps)
      use sizes
      use atoms
      use gkstuf
      use group
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine induce0e_aniso  --  Poisson-Boltzmann induced dipoles  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "induce0e_aniso" computes the induced dipole moments at polarizable
c     sites for Poisson-Boltzmann SCRF and vacuum environments
c
c
      subroutine induce0e_aniso
      use sizes
      use atoms
      use inform
      use iounit
...
c     another set that also includes RF due to permanent multipoles
c
      call dfield0e_aniso (field,fieldp,fields,fieldps)
c
c     set vacuum induced dipoles to polarizability times direct field;
...
c     set initial conjugate gradient residual and conjugate vector
c
         call ufield0e_aniso (field,fieldp,fields,fieldps)
         do i = 1, npole
            poli(i) = max(polmin,polarity(i))
            do j = 1, 3
               rsd(j,i) = (udir(j,i)-uind(j,i))/poli(i)
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine dfield0e_aniso  --  Poisson-Boltzmann direct field  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "dfield0e_aniso" computes the direct electrostatic field due to
c     permanent multipole moments for use with in Poisson-Boltzmann
c
c
      subroutine dfield0e_aniso (field,fieldp,fields,fieldps)
      use sizes
      use atoms
      use couple
      use group
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine ufield0e_aniso  --  Poisson-Boltzmann mutual field  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "ufield0e_aniso" computes the mutual electrostatic field due to
c     induced dipole moments via a Poisson-Boltzmann solver
c
c
      subroutine ufield0e_aniso (field,fieldp,fields,fieldps)
      use sizes
      use atoms
      use group
      use mpole
...
c     ################################################################
c     ##                                                            ##
c     ##  subroutine ulspred_aniso  --  induced dipole prediction coeffs  ##
c     ##                                                            ##
c     ################################################################
c
c
c     "ulspred_aniso" uses standard extrapolation or a least squares fit
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine uscale0a_aniso  --  dipole preconditioner via loop  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "uscale0a_aniso" builds and applies a preconditioner for the conjugate
...
c
         polmin = 0.00000001d0
c         do i = 1, npole
c            poli = udiag*max(polmin,
c     &                   ((anisopolarity(1,i)+anisopolarity(5,i)+
c     &                     anisopolarity(9,i))/3.0d0))
cc           poli = udiag * max(polmin,polarity(i))
c            do j = 1, 3
...
c         end do
         do i = 1, npole
               zrsd(1,i) = ranisopolarity(1,i)*rsd(1,i)
     &                   + ranisopolarity(2,i)*rsd(2,i)
     &                   + ranisopolarity(3,i)*rsd(3,i)
               zrsd(2,i) = ranisopolarity(4,i)*rsd(1,i)
     &                   + ranisopolarity(5,i)*rsd(2,i)
     &                   + ranisopolarity(6,i)*rsd(3,i)
               zrsd(3,i) = ranisopolarity(7,i)*rsd(1,i)
     &                   + ranisopolarity(8,i)*rsd(2,i)
     &                   + ranisopolarity(9,i)*rsd(3,i)
              zrsdp(1,i) = ranisopolarity(1,i)*rsdp(1,i)
     &                   + ranisopolarity(2,i)*rsdp(2,i)
     &                   + ranisopolarity(3,i)*rsdp(3,i)
              zrsdp(2,i) = ranisopolarity(4,i)*rsdp(1,i)
     &                   + ranisopolarity(5,i)*rsdp(2,i)
     &                   + ranisopolarity(6,i)*rsdp(3,i)
              zrsdp(3,i) = ranisopolarity(7,i)*rsdp(1,i)
     &                   + ranisopolarity(8,i)*rsdp(2,i)
     &                   + ranisopolarity(9,i)*rsdp(3,i)
         end do
c
...
c
         ...
         do i = 1, npole-1
            ...
            pti = thole(i)
c            poli = polarity(i)
c            poli = (anisopolarity(1,i)+anisopolarity(5,i)+
c     &                     anisopolarity(9,i))/3.0d0
            poli = (ranisopolarity(1,i)+ranisopolarity(5,i)+
     &                     ranisopolarity(9,i))/3.0d0
            do j = i+1, npole
               dscale(ipole(j)) = 1.0d0
            end do
            do j = 1, np11(ii)
...
c                  polik = poli * polarity(k)
c                   polik = poli*((anisopolarity(1,k)+anisopolarity(5,k)+
c     &                     anisopolarity(9,k))/3.0d0)
                 polik = poli*((ranisopolarity(1,k)+ranisopolarity(5,k)+
     &                     ranisopolarity(9,k))/3.0d0)
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine uscale0b_aniso  --  dipole preconditioner via list  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "uscale0b_aniso" builds and applies a preconditioner for the conjugate
c     gradient induced dipole solver using a neighbor pair list
c
c
      subroutine uscale0b_aniso (mode,rsd,rsdp,zrsd,zrsdp)
      use sizes
      use atoms
      use mpole
      use neigh
...
c
         polmin = 0.00000001d0
         do i = 1, npole
c            poli = udiag * max(polmin,polarity(i))
            poli = udiag*max(polmin,
     &                   ((anisopolarity(1,i)+anisopolarity(5,i)+
     &                     anisopolarity(9,i))/3.0d0))
            do j = 1, 3
               zrsd(j,i) = 0.0d0
               zrsdp(j,i) = 0.0d0
...
!$OMP DO schedule(guided)
         do i = 1, npole
            ...
            pti = thole(i)
c            poli = polarity(i)
            poli = (anisopolarity(1,i)+anisopolarity(5,i)+
     &                     anisopolarity(9,i))/3.0d0
            do j = 1, np11(ii)
               dscale(ip11(j,ii)) = u1scale
            end do
...
c               polik = poli * polarity(k)
                   polik = poli*((anisopolarity(1,k)+anisopolarity(5,k)+
     &                     anisopolarity(9,k))/3.0d0)
               rr3 = scale3 * polik / (r*r2)
               rr5 = 3.0d0 * scale5 * polik / (r*r2*r2)
               minv(m+1) = rr5*xr*xr - rr3
               minv(m+2) = rr5*xr*yr
