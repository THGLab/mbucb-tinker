...
c
      ...
      real*8 alpha(3,3)
      real*8 valpha(3,3)
      real*8 ranisopol_mat(9,9)
c      real*8 anisopolarity(9,n)
      character*40 fstr
c
c
...
c
      ...
      call mechanic
c        do i=1,npole
c           write(*,37)(anisopolarity(j,i),j=1,9)
c   37    format (9f7.4)
c        end do
c
c     sum atomic polarizabilities to get additive molecular value
c
      if (.not. use_polar .and. use_anisopolz) then
         write (iout,10)
     ...
     &              ' is Not in Use')
      ...
      do i = 1, npole
         if(use_anisopolz) then
           addu=anisopolarity(1,i)+anisopolarity(2,i)+anisopolarity(3,i)
     &        + anisopolarity(4,i)+anisopolarity(5,i)+anisopolarity(6,i)
     &        + anisopolarity(7,i)+anisopolarity(8,i)+anisopolarity(9,i)
     &        + addu
         else
...
c
      ...
      exfield(1) = external
         if(use_anisopolz) then
      call moluind_aniso (exfield,umol)
         else
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine moluind_aniso  --  molecular induced dipole in field  ##
c     ##                                                             ##
c     #################################################################
c
c
...
c
c
      subroutine moluind_aniso (exfield,umol)
      use sizes
      use atoms
      use inform
      use iounit
...
c
      do i = 1, npole
        do j=1,3
            uind(1,i) = anisopolarity(1,i)*exfield(j) +
     &      anisopolarity(2,i)*exfield(j) 
     &      + anisopolarity(3,i)*exfield(j)

            uind(2,i) = anisopolarity(4,i)*exfield(j) + 
     &      anisopolarity(5,i)*exfield(j) 
     &      + anisopolarity(6,i)*exfield(j)

            uind(3,i) = anisopolarity(7,i)*exfield(j) +
     &      anisopolarity(8,i)*exfield(j)
     &      + anisopolarity(9,i)*exfield(j)
            uind(j,i) = uind(j,i)
            end do
      end do
c
...
c
      if (poltyp .eq. 'MUTUAL') then
         ...
         call ufield (field)
         do i = 1, npole
           polnorm(i)=sqrt(anisopolarity(1,i)**2+anisopolarity(2,i)**2+
     &              anisopolarity(3,i)**2+anisopolarity(4,i)**2+
     &              anisopolarity(5,i)**2+anisopolarity(6,i)**2+
     &              anisopolarity(7,i)**2+anisopolarity(8,i)**2+
     &              anisopolarity(9,i)**2)
c            poli(i) =max(polmin,((anisopolarity(1,i)+anisopolarity(5,i)+
c     &           anisopolarity(9,i))/3.0d0))
            poli(i) =max(polmin,polnorm(i))
            do j = 1, 3
               rsd(j,i) = field(j,i)
               zrsd(j,i) = rsd(j,i) * poli(i)
               conj(j,i) = zrsd(j,i)
...
c
          ...
          write(*,*)'ufield debugg',i,k,field(1,i),field(2,i),field(3,i)
         end do
c
