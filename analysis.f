...
c      if (use_mpole .or. use_polar)  call empole3
!          if(usepiquemalcdq) then
        if (ecp%use_chgpen) then
!      if (use_mpole .or. use_polar)  call empole3_pen
            if (use_mpole .or. use_polar)  call empole3_cp
         else
            if (use_mpole .or. use_polar)  call empole3
         end if
      if (use_mpole .or. use_polar.or.use_chargetranfer)call empole3_ct
...
c
     ...
     &          + em + ep + er + es + elf + eg + ex + ep_ct
      energy = esum
c      print*,"PolEng=",ep
c      print*,"CT_Eng=",ep_ct
c
