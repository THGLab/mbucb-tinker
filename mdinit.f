...
c
      do i = 1, nkey
         ...
         if (keyword(1:11) .eq. 'INTEGRATOR ') then
            ...
            call upcase (polpred)
c akd
         else if (keyword(1:14) .eq. 'CT-PREDICT ') then
            use_pred_ct = .true.
            call getword (record,polpred_ct,next)
            call upcase (polpred_ct)
c akd
...
c akd
      ...
      if (use_pred_ct) then
      if (.not.allocated(udalt_ct))  allocate (udalt_ct(maxualt_ct,3,n))
      if (.not.allocated(upalt_ct))  allocate (upalt_ct(maxualt_ct,3,n))
      if (.not.allocated(usalt_ct))  allocate (usalt_ct(maxualt_ct,3,n))
      if (.not.allocated(upsalt_ct))allocate (upsalt_ct(maxualt_ct,3,n))
c
c     set the Gear predictor binomial coefficients
c
         gear_ct(1) = 6.0d0
...
c     set always stable predictor-corrector (ASPC) coefficients
c
         aspc_ct(1) = 22.0d0 / 7.0d0
         aspc_ct(2) = -55.0d0 / 14.0d0
         aspc_ct(3) = 55.0d0 / 21.0d0
         aspc_ct(4) = -22.0d0 / 21.0d0
         aspc_ct(5) = 5.0d0 / 21.0d0
         aspc_ct(6) = -1.0d0 / 42.0d0
         aspc_ct(7) = 0.0d0
c
c    initialize prior values of induced dipole moments
c
         ...
         do i = 1, npole
            do j = 1, 3
               do k = 1, maxualt_ct
                  udalt_ct(k,j,i) = 0.0d0
                  upalt_ct(k,j,i) = 0.0d0
                  usalt_ct(k,j,i) = 0.0d0
                  upsalt_ct(k,j,i) = 0.0d0
               end do
            end do
         end do
      end if
