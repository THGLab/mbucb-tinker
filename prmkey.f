...
c qtw >> in
c     charge penetration switch
      else if (keyword(1:11) .eq. 'CHGPENTERM ') then
         call getword (record,value,next)
         if (value .eq. 'ONLY')  call potoff
         ecp%use_chgpen = .true.
         if (value .eq. 'NONE')  ecp%use_chgpen = .false.
c qtw << out
...
c qtw >> in
c     charge penetration type
      else if (keyword(1:11) .eq. 'CHGPEN-TYPE ') then
         call getword (record,chgpen_tmp,next)
         if (chgpen_tmp(1:3) .eq. 'JP1') then
            ecp%chgpen_form = 1
         end if
c qtw << out
