...
c
         do i = 1, npole
            poli(i) =max(polmin,((anisopolarity(1,i)+anisopolarity(5,i)+
...
c
         do while (.not. done)
          ...
          if(use_anisopolz) then
             write (iout,101)
  101      format (/,' Anisotropic polarizability is not yet available',
     &              ' for Particle Mesh Ewald with Neighbor Lists')
             call fatal
            else
               call ufield0c_aniso (field,fieldp)
          ...
          if(use_anisopolz) then
             write (iout,101)
  102      format (/,' Anisotropic polarizability is not yet available',
     &              ' for Neighbor Lists')
             call fatal
            else
               call ufield0b_aniso (field,fieldp)
...
c            if (use_mlist) then
c               call uscale0b_aniso (mode,rsd,rsdp,zrsd,zrsdp)
c           else
               call uscale0a_aniso (mode,rsd,rsdp,zrsd,zrsdp)
c            end if
            b = 0.0d0
