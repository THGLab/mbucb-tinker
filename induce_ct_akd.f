...
c
      if (use_ewald) then
        ...
        call dfield0b_ct (field_ct,fieldp_ct)
      else
...
c
         if (use_ewald) then
            ...
            call ufield0b_ct (field_ct,fieldp_ct)
         else
...
c
     ...
     &                       + fieldp_ct(j,i)
         ...
         mode = 'BUILD'
         if (use_mlist) then
            call uscale0b_ct (mode,rsd_ct,rsdp_ct,zrsd_ct,zrsdp_ct)
            mode = 'APPLY'
            call uscale0b_ct (mode,rsd_ct,rsdp_ct,zrsd_ct,zrsdp_ct)
         else
...
c
   ...
   20          format (i8,7x,f16.10)
            ...
            if (iter_ct .ge. politer) done = .true.
         end do
c
...
c
     ...
     &                        - rr3*diz - 2.0d0*rr5*qiz
                  do j = 1, 3
                 field_ct(j,i) = field_ct(j,i) + fid_ct(j)*dscale(kk)
                 field_ct(j,k) = field_ct(j,k) + fkd_ct(j)*dscale(kk)
               fieldp_ct(j,i) = fieldp_ct(j,i) + fid_ct(j)*pscale(kk)
               fieldp_ct(j,k) = fieldp_ct(j,k) + fkd_ct(j)*pscale(kk)
                  end do
               end if
            end if
         end do
...
c
     ...
     &                        - rr3*diz - 2.0d0*rr5*qiz
                     ...
                     if (use_polymer .and. r2 .le. polycut2) then
                        do j = 1, 3
                           fid_ct(j) = fid_ct(j) * dscale(kk)
                           fip_ct(j) = fip_ct(j) * pscale(kk)
                           fkd_ct(j) = fkd_ct(j) * dscale(kk)
                           fkp_ct(j) = fkp_ct(j) * pscale(kk)
                        end do
                     end if
                     do j = 1, 3
                     ...
                     if (use_polymer) then
                        if (r2 .le. polycut2) then
                           do j = 1, 3
                              fid_ct(j) = fid_ct(j) * dscale(kk)
                              fkd_ct(j) = fkd_ct(j) * dscale(kk)
                              fip_ct(j) = fip_ct(j) * dscale(kk)
                              fkp_ct(j) = fkp_ct(j) * dscale(kk)
                           end do
                        end if
                     end if
                     do j = 1, 3
...
c
c
      subroutine dfield0b_ct (field_ct,fieldp_ct)
      use sizes
      use atoms
      use bound
      use couple
...
c
c
      subroutine ufield0b_ct (field_ct,fieldp_ct)
      use sizes
      use atoms
      use bound
      use group
...
c
               ...
               qkz = qkxz*xr + qkyz*yr + qkzz*zr
               qkr = qkx*xr + qky*yr + qkz*zr
             bcn(1) = bn(1) - (1.0d0-scale3_ct*dscale(kk))*rr3
             bcn(2) = bn(2) - 3.0d0*(1.0d0-scale5_ct*dscale(kk))*rr5
             bcn(3) = bn(3) - 15.0d0*(1.0d0-scale7_ct*dscale(kk))*rr7
     ...
     &                     - bcn(1)*diz - 2.0d0*bcn(2)*qiz
             bcn(1) = bn(1) - (1.0d0-scale3_ct*pscale(kk))*rr3
             bcn(2) = bn(2) - 3.0d0*(1.0d0-scale5_ct*pscale(kk))*rr5
             bcn(3) = bn(3) - 15.0d0*(1.0d0-scale7_ct*pscale(kk))*rr7
...
c
     ...
     &                           *(1.0d0-damp_ct+0.6d0*damp_ct**2)
                     ...
                     if (use_polymer) then
                        if (r2 .le. polycut2) then
                           dsc3 = scale3_ct * dscale(kk)
                           dsc5 = scale5_ct * dscale(kk)
                           dsc7 = scale7_ct * dscale(kk)
                           psc3 = scale3_ct * pscale(kk)
                           psc5 = scale5_ct * pscale(kk)
                           psc7 = scale7_ct * pscale(kk)
                        end if
                     end if
c
c     find the field terms for the current interaction
...
c
c
      subroutine udirect2b_ct (field_ct,fieldp_ct)
      use sizes
      use atoms
      use boxes
      use bound
...
c
c!$OMP PARALLEL default(private) shared(n,npole,ipole,x,y,z,pdamp,thole,
...
c!$OMP& toffset,toffset0,field_ct,fieldp_ct,fieldt_ct,fieldtp_ct,maxlocal)
...
c     compute the real space portion of the Ewald summation
c
c!$OMP DO reduction(+:fieldt,fieldtp) schedule(guided)
      do i = 1, npole
         ii = ipole(i)
         pdi_ct = pdamp_ct(i)
         pti_ct = thole_ct(i)
...
c
               if (poltyp .eq. 'MUTUAL') then
                 bcn(1) = bn(1) - (1.0d0-scale3_ct*uscale(kk))*rr3
                 bcn(2) = bn(2) - 3.0d0*(1.0d0-scale5_ct*uscale(kk))*rr5
                  nlocal = nlocal + 1
                  ilocal(1,nlocal) = i
                  ilocal(2,nlocal) = k
...
c
     ...
     &                              *(1.0d0-damp_ct)
                     ...
                     if (use_polymer) then
                        if (r2 .le. polycut2) then
                           scale3_ct = scale3_ct * uscale(kk)
                           scale5_ct = scale5_ct * uscale(kk)
                        end if
                     end if
                     bn(1) = -bn(1) + (1.0d0-scale3_ct)*rr3
                     bn(2) = bn(2) - 3.0d0*(1.0d0-scale5_ct)*rr5
...
c
      ...
      real*8 scale3_ct,scale5_ct
      real*8 m1_ct,m2_ct,m3_ct
      real*8 m4_ct,m5_ct,m6_ct
      real*8 m1,m2,m3
      real*8 m4,m5,m6
      real*8, allocatable :: dscale(:)
      real*8 rsd_ct(3,*)
...
c
         polmin_ct = 0.00000001d0
         do i = 1, npole
            poli_ct = udiag * max(polmin_ct,charge_trans(i))
            do j = 1, 3
               ...
               if (r2 .le. off2) then
                  ...
                  if (damp_ct .ne. 0.0d0) then
                     ...
                     if (damp_ct .gt. -50.0d0) then
                        expdamp_ct = exp(damp_ct)
                scale3_ct = scale3_ct * (1.0d0-expdamp_ct*damp_beta_ct)
                scale5_ct = scale5_ct * (1.0d0-damp_beta_ct*
     &                           expdamp_ct*(1.0d0-damp_ct))
                     end if
                  end if
...
c
c
      subroutine uscale0b_ct (mode,rsd_ct,rsdp_ct,zrsd_ct,zrsdp_ct)
      use sizes
      use atoms
      use mpole
      use neigh
      ...
      real*8 zrsd_ct(3,*)
      real*8 zrsdp_ct(3,*)
      real*8, allocatable :: zrsdt_ct(:,:)
      real*8, allocatable :: zrsdtp_ct(:,:)
      character*6 mode
c
c
c     apply the preconditioning matrix to the current residual
...
c     perform dynamic allocation of some local arrays
c
         allocate (zrsdt_ct(3,npole))
         allocate (zrsdtp_ct(3,npole))
c
c     use diagonal preconditioner elements as first approximation
c
         polmin_ct = 0.00000001d0
         do i = 1, npole
            ...
            do j = 1, 3
               zrsd_ct(j,i) = 0.0d0
               zrsdp_ct(j,i) = 0.0d0
               zrsdt_ct(j,i) = poli_ct * rsd_ct(j,i)
               zrsdtp_ct(j,i) = poli_ct * rsdp_ct(j,i)
            end do
         end do
c
c     use the off-diagonal preconditioner elements in second phase
c
c!$OMP PARALLEL default(private) shared(npole,mindex,minv,nulst,ulst,
c!$OMP& rsd,rsdp,zrsd,zrsdp,zrsdt,zrsdtp)
c!$OMP DO reduction(+:zrsdt,zrsdtp) schedule(guided)
         do i = 1, npole
            ...
            do kk = 1, nulst(i)
               ...
               m6 = minv(m+6)
               m = m + 6
        zrsdt_ct(1,i) = zrsdt_ct(1,i) + m1*rsd_ct(1,k) + m2*rsd_ct(2,k)
     &                 + m3*rsd_ct(3,k)
        zrsdt_ct(2,i) = zrsdt_ct(2,i) + m2*rsd_ct(1,k) + m4*rsd_ct(2,k)
     &                 + m5*rsd_ct(3,k)
        zrsdt_ct(3,i) = zrsdt_ct(3,i) + m3*rsd_ct(1,k) + m5*rsd_ct(2,k)
     &                 + m6*rsd_ct(3,k)
        zrsdt_ct(1,k) = zrsdt_ct(1,k) + m1*rsd_ct(1,i) + m2*rsd_ct(2,i)
     &                 + m3*rsd_ct(3,i)
        zrsdt_ct(2,k) = zrsdt_ct(2,k) + m2*rsd_ct(1,i) + m4*rsd_ct(2,i)
     &                 + m5*rsd_ct(3,i)
        zrsdt_ct(3,k) = zrsdt_ct(3,k) + m3*rsd_ct(1,i) + m5*rsd_ct(2,i)
     &                 + m6*rsd_ct(3,i)
        zrsdtp_ct(1,i)= zrsdtp_ct(1,i) + m1*rsdp_ct(1,k)+m2*rsdp_ct(2,k)
     &                 + m3*rsdp_ct(3,k)
        zrsdtp_ct(2,i)= zrsdtp_ct(2,i) + m2*rsdp_ct(1,k)+m4*rsdp_ct(2,k)
     &                 + m5*rsdp_ct(3,k)
        zrsdtp_ct(3,i)= zrsdtp_ct(3,i) + m3*rsdp_ct(1,k)+m5*rsdp_ct(2,k)
     &                 + m6*rsdp_ct(3,k)
        zrsdtp_ct(1,k)= zrsdtp_ct(1,k) + m1*rsdp_ct(1,i)+m2*rsdp_ct(2,i)
     &                 + m3*rsdp_ct(3,i)
        zrsdtp_ct(2,k)= zrsdtp_ct(2,k) + m2*rsdp_ct(1,i)+m4*rsdp_ct(2,i)
     &                 + m5*rsdp_ct(3,i)
        zrsdtp_ct(3,k)= zrsdtp_ct(3,k) + m3*rsdp_ct(1,i)+m5*rsdp_ct(2,i)
     &                  + m6*rsdp_ct(3,i)
            end do
         end do
c!$OMP END DO
...
c!$OMP DO
         do i = 1, npole
            do j = 1, 3
               zrsd_ct(j,i) = zrsdt_ct(j,i) + zrsd_ct(j,i)
               zrsdp_ct(j,i) = zrsdtp_ct(j,i) + zrsdp_ct(j,i)
            end do
         end do
c!$OMP END DO
c!$OMP END PARALLEL
...
c     set OpenMP directives for the major loop structure
c
c!$OMP PARALLEL default(private) shared(n,npole,ipole,x,y,z,pdamp,
c!$OMP& thole,polarity,u1scale,u2scale,u3scale,u4scale,np11,ip11,
c!$OMP& np12,ip12,np13,ip13,np14,ip14,nulst,ulst,mindex,minv)
            ...
            do kkk = 1, nulst(i)
               ...
               if (damp_ct .ne. 0.0d0) then
                  ...
                  if (damp_ct .gt. -50.0d0) then
                     expdamp_ct = exp(damp_ct)
                scale3_ct = scale3_ct * (1.0d0-expdamp_ct*damp_beta_ct)
                scale5_ct = scale5_ct * (1.0d0-expdamp_ct*
