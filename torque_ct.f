...
c
c
      subroutine torque_ct (i,trq2,frcx,frcy,frcz)
      use sizes
      use atoms
      use deriv
      use mpole
...
c         end do
cc
cc     force distribution for the Bisector local coordinate method
cc
...
c         end do
cc
cc     force distribution for the Z-Bisect local coordinate method
cc
...
c         end do
cc
cc     force distribution for the 3-Fold local coordinate method
cc        (correct for uv, uw and vw angles all equal to 90)
cc
...
c
      if (axetyp .eq. 'Z-Only') then
         do j = 1, 3
            ...
            dep_ct(j,ia) = dep_ct(j,ia) + du
            dep_ct(j,ib) = dep_ct(j,ib) - du
            frcz(j) = frcz(j) + du
         end do
c
c     force distribution for the Z-then-X local coordinate method
c
         ...
         do j = 1, 3
            ...
            dep_ct(j,ia) = dep_ct(j,ia) + du
            dep_ct(j,ic) = dep_ct(j,ic) + dv
            dep_ct(j,ib) = dep_ct(j,ib) - du - dv
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
         end do
c
...
c
     ...
     &              / (wsiz*(ut1sin+ut2sin))
            dep_ct(j,ia) = dep_ct(j,ia) + du
            dep_ct(j,ic) = dep_ct(j,ic) + dv
            dep_ct(j,id) = dep_ct(j,id) + dw
            dep_ct(j,ib) = dep_ct(j,ib) - du - dv - dw
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
            frcy(j) = frcy(j) + dw
         end do
...
c
c
      subroutine torque2_ct (trq,derivs)
      use sizes
      use atoms
      use mpole
      implicit none
...
c
c
      subroutine torque3_ct (i,trq2,frcx,frcy,frcz,depi)
      use sizes
      use atoms
      use mpole
      implicit none
