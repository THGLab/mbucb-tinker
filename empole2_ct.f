...
c      if (.not. allocated(dem))  allocate (dem(3,n))
      if (.not. allocated(dep_ct))  allocate (dep_ct(3,n))
c
...
c
      if (.not. twosided) then
         call empole2a_ct (nlist,list,reinduce)
         do k = 1, n
            do j = 1, 3
               d0(j,k) = dep_ct(j,k)
            end do
...
c
      ...
      do k = 1, n
         do j = 1, 3
            hessx(j,k) = hessx(j,k) + (dep_ct(j,k)-d0(j,k))/eps
         end do
      end do
c
...
c
      ...
      do k = 1, n
         do j = 1, 3
            hessy(j,k) = hessy(j,k) + (dep_ct(j,k)-d0(j,k))/eps
         end do
      end do
c
...
c
      ...
      do k = 1, n
         do j = 1, 3
            hessz(j,k) = hessz(j,k) + (dep_ct(j,k)-d0(j,k))/eps
         end do
      end do
c
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine empole2a_ct  --  mpole & polar Hessian; numerical  ##
c     ##                                                             ##
c     #################################################################
c
c
c     "empole2a_ct" computes multipole and dipole polarization first
c     derivatives for a single atom with respect to Cartesian
c     coordinates; used to get finite difference second derivatives
c
c
      subroutine empole2a_ct (nlist,list,reinduce)
      use sizes
      use atoms
      use bound
      use boxes
