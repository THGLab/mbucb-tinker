...
c
         if (keyword(1:6) .eq. 'EWALD ') then
            use_ewald = .true.
         else if(keyword(1:12) .eq. 'PIQUEMALCDQ ') then
            usepiquemalcdq = .true.
         else if (keyword(1:13) .eq. 'CHARGETRANSF ') then
            use_chargetranfer = .true.
            print*,'use_chargetranfer=',use_chargetranfer
...
c
      ...
      if (use_mlist) then
         ...
         if (poltyp .eq. 'MUTUAL') then
            ...
            if (allocated(tindex_ct))  deallocate (tindex_ct)
            if (allocated(tdipdip_ct))  deallocate (tdipdip_ct)
            allocate (tindex_ct(2,n*maxelst))
            allocate (tdipdip_ct(6,n*maxelst))
         end if
      end if
      if (use_ulist) then
