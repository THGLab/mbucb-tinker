...
c
      if (use_ewald) then
         call dfield0c_ct (field,fieldp)
      else if (use_mlist) then
         call dfield0b_ct (field,fieldp)
      else
...
c
      do i = 1, npole
         do j = 1, 3
            udir(j,i) = charge_trans(i) * field(j,i)
            udirp(j,i) = charge_trans(i) * fieldp(j,i)
            uind(j,i) = udir(j,i)
            uinp(j,i) = udirp(j,i)
         end do
      end do
cc
cc     set tolerances for computation of mutual induced dipoles
cc
...
c         eps = 100.0d0
cc
cc     estimated induced dipoles from polynomial predictor
cc
...
cc
cc     get the electrostatic field due to induced dipoles
cc
c         if (use_ewald) then
c            call ufield0c_ct (field,fieldp)
c         else if (use_mlist) then
c            call ufield0b_ct (field,fieldp)
c         else
c            call ufield0a_ct (field,fieldp)
c         end if
cc
cc     set initial conjugate gradient residual and conjugate vector
cc
c         do i = 1, npole
c            poli(i) = max(polmin,charge_trans(i))
c            do j = 1, 3
...
c            call uscale0b_ct (mode,rsd,rsdp,zrsd,zrsdp)
c         else
c            call uscale0a_ct (mode,rsd,rsdp,zrsd,zrsdp)
c            mode = 'APPLY'
c            call uscale0a_ct (mode,rsd,rsdp,zrsd,zrsdp)
c         end if
c         do i = 1, npole
...
c            write (iout,40)
c   40       format (/,' INDUCE  --  Warning, Induced Charge Transfer',
c     &                 ' are not Converged')
c            call prterr
c            call fatal
c         end if
...
c
c
      subroutine ufield0a_ct (field,fieldp)
      use sizes
      use atoms
      use bound
      use cell
...
c
c
      subroutine dfield0c_ct (field,fieldp)
      use sizes
      use atoms
      use boxes
      use ewald
...
c
      if (use_mlist) then
         call udirect2b_ct (field,fieldp)
      else
         call udirect2a_ct (field,fieldp)
      end if
c
...
c
c
      subroutine ufield0c_ct (field,fieldp)
      use sizes
      use atoms
      use boxes
      use ewald
...
c     get the reciprocal space part of the electrostatic field
c
      call umutual1_ct (field,fieldp)
c
c     get the real space portion of the electrostatic field
c
      if (use_mlist) then
         call umutual2b_ct (field,fieldp)
      else
         call umutual2a_ct (field,fieldp)
      end if
c
c     get the self-energy portion of the electrostatic field
c
...
c
c
      subroutine udirect2a_ct (field,fieldp)
      use sizes
      use atoms
      use boxes
      use bound
...
c
c
      subroutine umutual1_ct (field,fieldp)
      use sizes
      use boxes
      use ewald
      use math
...
c
c
      subroutine umutual2a_ct (field,fieldp)
      use sizes
      use atoms
      use boxes
      use bound
...
c
c
      subroutine umutual2b_ct (field,fieldp)
      use sizes
      use mpole
      use polar
      use tarray
...
c
c
      subroutine uscale0a_ct (mode,rsd,rsdp,zrsd,zrsdp)
      use sizes
      use atoms
      use limits
      use mpole
