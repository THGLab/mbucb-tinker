...
c
c                  muse = (use_mpole .and. mscale(kk).ne.0.0d0)
...
c!$OMP& damp,expdamp,pdi,pti,pgamma,scale3,scale5,scale7,xr,yr,zr,r,r2,
...
c                  e = gl(0)*rr1 + gl(1)*rr3 + gl(2)*rr5
c     &                   + gl(3)*rr7 + gl(4)*rr9
                  ei_ct = gli(1)*rr3*scale3 + gli(2)*rr5*scale5
     &                    + gli(3)*rr7*scale7
c
c     apply the energy adjustments for scaled interactions
c
...
!$OMP& private(i,j,k,ii,kk,kkk,e,ei_ct,efix,eifix,bfac,damp,expdamp,
...
c
               ...
               if (damp .ne. 0.0d0) then
                  ...
                  if (damp .gt. -50.0d0) then
                     ...
                     scale5 = scale5 *
     &                    (1.0d0-(1.0d0-damp)*expdamp*damp_beta)
                     scale7 = scale7 * (1.0d0-(1.0d0-damp
     &
     &                        +0.6d0*damp**2)*expdamp*damp_beta)
                  end if
               end if
c               efix = gl(0)*rr1 + gl(1)*rr3 + gl(2)*rr5
c     &                   + gl(3)*rr7 + gl(4)*rr9
